...
using static Litchi.Output;
using FileUtil = UnityEngine.AssetGraph.FileUtility;

namespace Litchi {

	[CustomNode("Build/Build Asset Bundle 22", 90)]
	public class AssetBundleMapBuilder22 : Node, Model.NodeDataImporter {
		...
		public override Node Clone(Model.NodeData newData) {
			var newNode = new AssetBundleMapBuilder22();
			...
			PerformGraph.Output Output) 
		{
			...
			UnityEngine.Debug.Log("[Lei] bundleOutputDir " + " " + bundleOutputDir);

			var bundleNames = incoming.SelectMany(v => v.assetGroups.Keys).Distinct().ToList();
			Dump(bundleNames, "bundleNames");
			...
			UnityEngine.Debug.Log("[Lei] manifestName " + " " + manifestName);
			bundleNames.Add( manifestName );
			bundleVariants[manifestName] = new List<string>() {""};
			
			Dump(connectionsToOutput, "connectionsToOutput ");
			Dump(Output, "Output");
			if(connectionsToOutput != null && Output != null) {
				...
				foreach (var name in bundleNames) {
					foreach(var v in bundleVariants[name]) {
						...
						AssetReference bundle = AssetReferenceDatabase.GetAssetBundleReference( FileUtil.PathCombine(bundleOutputDir, bundleName) );
						AssetReference manifest = AssetReferenceDatabase.GetAssetBundleReference( FileUtil.PathCombine(bundleOutputDir, bundleName + Model.Settings.MANIFEST_FOOTER) );
						outputDict[key].Add(bundle);
						outputDict[key].Add(manifest);

						UnityEngine.Debug.Log("[Lei] iiiii111" + " " + bundle + "   ---   " + FileUtil.PathCombine(bundleOutputDir, bundleName));
						UnityEngine.Debug.Log("[Lei] iiiii222" + " " + manifest + "   ---   " + FileUtil.PathCombine(bundleOutputDir, bundleName + Model.Settings.MANIFEST_FOOTER));
					}
				}
			...
			Action<Model.NodeData, string, float> progressFunc) 
		{
			...
			foreach(var ag in incoming) {
				Dump(ag.assetGroups, "ag.assetGroups");
				foreach(var name in ag.assetGroups.Keys) {
			...
			Dump(bundleNames, "bundleNames222");
			var bundleVariants = new Dictionary<string, List<string>>();
...
UnityEngine.Debug.Log("[Lei] on build bundleOutputDir" + " " + bundleOutputDir);
            ...
            if (!string.IsNullOrEmpty (m_manifestName [target])) {
                ...
                var from = FileUtil.PathCombine (projectPath, bundleOutputDir, manifestName);
                var to = FileUtil.PathCombine (projectPath, bundleOutputDir, finalManifestName);
            ...
            var generatedFiles = FileUtil.GetAllFilePathsInFolder(bundleOutputDir);
			...
			foreach (var path in generatedFiles) {
				...
				if( IsFileIntendedItem(fileName, bundleVariants) ) {
                    if (fileName == manifestName) {
						...
						UnityEngine.Debug.Log("[Lei] sppp" + " " + path);
        ...
        private string PrepareOutputDirectory(BuildTarget target, Model.NodeData node, bool autoCreate, bool throwException) {
            ...
            if(outputOption == OutputOption.BuildInCacheDirectory) {
                return FileUtil.EnsureAssetBundleCacheDirExists (target, node);
            }
            ...
            if (autoCreate) {
                if(outputOption == OutputOption.DeleteAndRecreateOutputDirectory) {
                    if (Directory.Exists(outputDir)) {
                        FileUtil.DeleteDirectory(outputDir, true);
                    }
                }
